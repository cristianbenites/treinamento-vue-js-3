      ...
      >
        <div class="flex items-center">
          <span
            ...
            class="inline w-2 h-2 mr-2 rounded-full"
          /> {{ filter.label }}
        </div>
        <span
...
function applyFiltersSctructure (summary) {
...
export default {
  async setup (props, { emit }) {
    ...
    try {
      ...
      state.filters = applyFiltersSctructure(data)
    } catch (error) {
      state.hasError = !!error
      state.filters = applyFiltersSctructure({ all: 0, issue: 0, idea: 0, other: 0 })
    }
